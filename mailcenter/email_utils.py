...
def send_text_mail(subject: str, text_body: str, recipients, account_code="default"):
    """
    プレーンテキストメールを送信する共通関数。

    翻訳ブリッジの同期メールのように、本文へ機械可読JSONをマーカー付きで
    埋め込む用途で使う(HTML変換で本文が崩れるのを避けるため)。
    ...
    try:
        logger.info(
            f"[MAILCENTER] (text) SMTP 接続開始 host={smtp_host} port={smtp_port} "
            f"tls={use_tls} ssl={use_ssl} account_code={account_code}"
        )
        ...
        logger.info(
            f"[MAILCENTER] (text) メール送信完了: {len(recipients)} 件 → {recipients}"
        )
        return {"sent": True, "reason": ""}
    except Exception as e:
        logger.error(f"[MAILCENTER] (text) メール送信エラー: {e}", exc_info=True)
        return {"sent": False, "reason": f"SMTPエラー: {e}"}
