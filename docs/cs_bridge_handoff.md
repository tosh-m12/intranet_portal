...
### 6.1.1 Source of Truth と再送ポリシー(v2.2 で明文化)
- **社内 DB = 単一の正本**。Mac は読み取り + writeback クライアント。
- **writeback 失敗時の Mac 側仕様**(spec §6.4 を実装):
  1. スナップショット受信時、ローカル状態と照合して「反映されていない自分の op」を抽出
  2. 同 `op_id` を新 `nonce` のメールで**再送**(社内側は冪等管理)
  3. 最大 N 回(推奨 5 回) → 諦め、ローカル UI をスナップショットに**追従させる**
  4. バックオフ ≥ 5分
